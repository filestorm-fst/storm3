...
const helpers = require('../../fst3-core-helpers/export');
...
Accounts.prototype.signTransaction = function signTransaction(tx, privateKey, callback) {
  ...
  function signed(tx) {
    ...
    try {
      let transaction = helpers.formatters.inputCallFormatter(_.clone(tx));
...
Accounts.prototype.recoverTransaction = function recoverTransaction(rawTx) {
  let values = RLP.decode(rawTx);
  let signature = Account.encodeSignature(values.slice(6, 9));
  let recovery = Bytes.toNumber(values[6]);
  let extraData = recovery < 35 ? [] : [Bytes.fromNumber((recovery - 35) >> 1), '0x', '0x'];
  let signingData = values.slice(0, 6).concat(extraData);
  let signingDataHex = RLP.encode(signingData);
  return Account.recover(Hash.keccak256(signingDataHex), signature);
};
/* jshint ignore:end */

Accounts.prototype.hashMessage = function hashMessage(data) {
  let message = utils.isHexStrict(data) ? utils.hexToBytes(data) : data;
...
Accounts.prototype.sign = function sign(data, privateKey) {
  ...
  let signature = Account.sign(hash, privateKey);
...
Accounts.prototype.decrypt = function (v3Keystore, password, nonStrict) {
  ...
  let mac = utils.sha3(Buffer.concat([derivedKey.slice(16, 32), ciphertext])).replace('0x', '');
...
Accounts.prototype.encrypt = function (privateKey, password, options) {
  ...
  let ciphertext = Buffer.concat([cipher.update(Buffer.from(account.privateKey.replace('0x', ''), 'hex')), cipher.final()]);

  let mac = utils.sha3(Buffer.concat([derivedKey.slice(16, 32), Buffer.from(ciphertext, 'hex')])).replace('0x', '');
...
Wallet.prototype.remove = function (addressOrIndex) {
  let account = this[addressOrIndex];
...
Wallet.prototype.clear = function () {
  let _this = this;
  let indexes = this._currentIndexes();
...
Wallet.prototype.encrypt = function (password, options) {
  ...
  let accounts = indexes.map(function (index) {
...
Wallet.prototype.decrypt = function (encryptedWallet, password) {
  ...
  encryptedWallet.forEach(function (keystore) {
    let account = _this._accounts.decrypt(keystore, password);
