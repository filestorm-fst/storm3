...
Subscription.prototype.subscribe = function () {
  ...
  let payload = this._toPayload(args);
  ...
  if (!this.options.requestManager.provider) {
    let err1 = new Error('No provider set.');
  ...
  if (!this.options.requestManager.provider.on) {
    let err2 = new Error('The current provider doesn\'t support subscriptions: ' + this.options.requestManager.provider.constructor.name);
    ...
    this.options.requestManager.send({
      ...
      if (!err) {
        logs.forEach(function (log) {
          let output = _this._formatOutput(log);
      ...
      _this.options.requestManager.addSubscription(_this.id, payload.params[0], _this.options.type, function (err, result) {
        ...
        if (!err) {
          ...
          result.forEach(function (resultItem) {
            let output = _this._formatOutput(resultItem);
