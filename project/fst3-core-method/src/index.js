...
const Subscriptions = require('../../fst3-core-subscriptions/export').subscriptions;
...
Method.prototype._confirmTransaction = function (defer, result, payload) {
  ...
  let checkConfirmation = function (existingReceipt, isPolling, err, blockHeader, sub) {
    if (!err) {
  ...
  let startWatching = function (existingReceipt) {
...
const getWallet = function (from, accounts) {
  let wallet = null;
...
Method.prototype.buildCall = function () {
  ...
  let send = function () {
    let defer = promiEvent(!isSendTx),
      payload = method.toPayload(Array.prototype.slice.call(arguments));


    // CALLBACK function
    let sendTxCallback = function (err, result) {
      try {
    ...
    let sendSignedTx = function (sign) {

      let signedPayload = _.extend({}, payload, {
    ...
    let sendRequest = function (payload, method) {
      ...
      if (method && method.accounts && method.accounts.wallet && method.accounts.wallet.length) {
        ...
        if (payload.method === 'eth_sendTransaction') {
          ...
          // If wallet was found, sign tx, and send using sendRawTransaction
          if (wallet && wallet.privateKey) {
            let sign = method.accounts.sign(data, wallet.privateKey);
...
Method.prototype.request = function () {
  let payload = this.toPayload(Array.prototype.slice.call(arguments));
