...
const Subscription = require('../../fst3-core-subscriptions/export').subscription;
const formatters = require('../../fst3-core-helpers/export').formatters;
const errors = require('../../fst3-core-helpers/export').errors;
const promiEvent = require('../../fst3-core-promievent/export');
const abi = require('../../fst3-eth-abi/export');
...
Contract.prototype._getOrSetDefaultOptions = function getOrSetDefaultOptions(options) {
  let gasPrice = options.gasPrice ? String(options.gasPrice) : null;
  let from = options.from ? utils.toChecksumAddress(formatters.inputAddressFormatter(options.from)) : null;
...
Contract.prototype._decodeEventABI = function (data) {
  ...
  let argTopics = event.anonymous ? data.topics : data.topics.slice(1);
...
Contract.prototype._encodeMethodABI = function _encodeMethodABI() {
  let methodSignature = this._method.signature,
    args = this.arguments || [];
  ...
  if (methodSignature === 'constructor') {
    ...
    let returnValue = (signature) ? signature + paramsABI : paramsABI;
...
Contract.prototype._decodeMethodReturn = function (outputs, returnValues) {
  ...
  let result = abi.decodeParameters(outputs, returnValues);
...
Contract.prototype.deploy = function (options, callback) {
  ...
  let constructor = _.find(this.options.jsonInterface, function (method) {
...
Contract.prototype._generateEventOptions = function () {
  ...
  // get the callback
  let callback = this._getCallback(args);

  // get the options
  let options = (_.isObject(args[args.length - 1])) ? args.pop() : {};

  let eventName = (_.isString(args[0])) ? args[0] : 'allevents';
  let event = (eventName.toLowerCase() === 'allevents') ? {
    name: 'ALLEVENTS',
...
Contract.prototype._on = function () {
  let subOptions = this._generateEventOptions.apply(this, arguments);
...
Contract.prototype.getPastEvents = function () {
  ...
  let call = getPastLogs.buildCall();
        ...
        let extraFormatters = {
          receiptFormatter: function (receipt) {
            if (_.isArray(receipt.logs)) {
              ...
              // decode logs
              let events = _.map(receipt.logs, function (log) {
          ...
          contractDeployFormatter: function (receipt) {
            let newContract = _this._parent.clone();
